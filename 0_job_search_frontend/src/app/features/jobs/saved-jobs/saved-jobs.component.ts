...
@Component({
  ...
  template: `
    <app-page-header title="Saved Jobs" subtitle="Jobs you've bookmarked for later review." />

    @if (loading) {
      <app-loading-spinner message="Loading saved jobs..." />
    } @else if (!jobs.length) {
      <app-empty-state
        icon="🔖"
        title="No saved jobs yet"
        message="Save jobs from recommendations to review them here."
      />
    } @else {
      <div class="job-grid">
...
export class SavedJobsComponent implements OnInit {
  ...
  onSave(jobId: string): void {
    this.jobService.toggleSaveJob(jobId).subscribe(() => {
      this.jobService.getSavedJobs().subscribe((jobs) => (this.jobs = jobs));
    });
  }

  onApply(jobId: string): void {
    this.jobService.markAsApplied(jobId).subscribe((jobs) => {
      this.jobService.getSavedJobs().subscribe((saved) => (this.jobs = saved));
    });
  }
}
