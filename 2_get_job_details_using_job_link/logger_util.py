"""Colored console logging for job detail scraping."""
...
try:
    ...
    _COLORS = {
        ...
        "JOB_SKIP": Fore.YELLOW,
        "RESET": Style.RESET_ALL,
    }
except ImportError:
    _COLORS = {
        k: ""
        for k in ("STEP", "OK", "WARN", "ERR", "INFO", "DEBUG", "JOB", "JOB_NEW", "JOB_SKIP", "RESET")
    }
...
class ColorFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if getattr(record, "job_skip", False):
            return f"{_COLORS['JOB_SKIP']}{msg}{_COLORS['RESET']}"
...
def log_step(message: str, *args: Any) -> None:
    logging.getLogger("detail.step").info(message, *args, extra={"step": True})


def log_job_new(job_id: str, company: str, method: str) -> None:
    logging.getLogger("detail.job").info(
        "  SAVED [%s | %s] via %s",
        job_id,
        company,
        method,
...
def log_job_skip(job_id: str, company: str, reason: str) -> None:
    logging.getLogger("detail.job").info(
        "  SKIP  [%s | %s] %s",
        job_id,
        company,
